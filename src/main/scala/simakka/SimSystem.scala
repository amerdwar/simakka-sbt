...
//    val fromEntity = entities.get(from)
  ...
  def creatEntity(tp: String, name: String, id: Int): Props = tp match {
    ...
    case "Source" => Props(new simakka.test.Source(name, id))
    case "Server" => Props(new simakka.test.Server(name, id))
    case "CPU" => Props(new HDFS.CPU(name, id))
    case "CPUClient" => Props(new HDFS.CPUClient(name, id))
 ...
 def creatRemoteEntity(tp: String, name: String, id: Int,url: String): Props ={
  ...
  Props( new Server(name, id)).withDeploy(Deploy(scope=RemoteScope(one)))
    }
        case "Source" =>{
      val one = akka.actor.AddressFromURIString(url)
    
  Props( new Source(name, id)).withDeploy(Deploy(scope=RemoteScope(one)))
    }
...
//    simSystem ! EndOfSimulation
    ...
    simSystem ! InitEntity("entt1", Some(ArrayBuffer("""{"key": "value"}""")))
    sleep(1)
...
//    simSystem ! SimStartTest
  ...
  def startExperiment1(): Unit = {
    ...
    sleep(1)
    simSystem ! InitEntity("client", Some(ArrayBuffer("counter")))
